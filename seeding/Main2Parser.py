...
def exec_query(data_path, parser):
    ...
    file_list = FileIterator.split_multi_format(file_list, process_num=query_process_num)
    ...
    while True:
        added_twarr_block = FileIterator.multi_process(query_tw_file_multi,
                                                       [(file_slice, parser.__class__, parser.query_list,
                                                        ...
                                                        for file_slice in file_list[start * process_num: (
                                                                                                             start + 1) * process_num]])
...
def exec_train(seed_parser, unlb_parser, cntr_parser):
    et = EventTrainer()
    et.set_seed_twarr(FileIterator.load_array(seed_parser.get_query_result_file_name()))
    et.set_unlb_twarr(FileIterator.load_array(unlb_parser.get_query_result_file_name()))
    et.set_cntr_twarr(FileIterator.load_array(cntr_parser.get_query_result_file_name()))
    # localcounter, event_classifier = et.train_and_test()
    localcounter, event_classifier = et.train_and_test(et.seed_twarr, et.unlb_twarr, et.cntr_twarr)
...
def exec_pre_test(test_data_path):
    subfiles = FileIterator.listchildren(test_data_path, children_type='file')
    file_list = FileIterator.split_multi_format(
        [(test_data_path + file) for file in subfiles if file.endswith('.json')], process_num=15)
    twarr_blocks = FileIterator.multi_process(FileIterator.summary_unzipped_tweets_multi,
                                              [(file_list_slice,) for file_list_slice in file_list])
    twarr = FileIterator.merge_list(twarr_blocks)
    ...
    all_ids = set(FileIterator.load_array(test_data_path + 'test_ids_all.csv'))
    pos_ids = set(FileIterator.load_array(test_data_path + 'test_ids_pos.csv'))
    ...
    for tw in twarr:
        ...
        if twid in pos_ids:
            ...
            non_pos_dict[twid] = tw
    
    pos_twarr = list(pos_dict.values())
    non_pos_twarr = list(non_pos_dict.values())
    
    FileIterator.dump_array(test_data_path + 'pos_tweets.sum', pos_twarr)
    FileIterator.dump_array(test_data_path + 'non_pos_tweets.sum', non_pos_twarr)
